...
const WorldClock = () => {
  ...
  const handleSelect = () => {
    ...
    if (trimmedInputValue !== '' && targetTimeZones.includes(trimmedInputValue)) {
      setSelectedTimeZone(trimmedInputValue);
    } else {
      alert('Invalid time zone. Please select a valid time zone from the field.');
    }
  };
  return (
    <>
      <Header />
      <div className="bg-slate-500 h-[100vh]  flex flex-col items-center gap-20 worldparent-class">
        <h1 className="text-[80px] uppercase he-class">World Clock</h1>

        <div className="flex gap-[40px] btnparent-class">
          <input type="text" className="h-[50px] w-[370px] rounded-lg text-[25px] text-center hover:bg-emerald-200 text-[red] bg-sky-800 inp-class"
            list="timeZonesList"
            value={inputValue}
